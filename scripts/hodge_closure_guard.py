#!/usr/bin/env python3
"""Canonical-lane closure guard for Hodge conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    kappa_hodge, t1 = _entry(consts, "kappa_hodge")
    ...
    alpha_alg, t5 = _entry(consts, "alpha_alg")
    a_floor, t6 = _entry(consts, "a_floor")
    eps_coh, t7 = _entry(consts, "eps_coh")

    h_g1 = t1 and _finite(kappa_hodge) and float(kappa_hodge) > 0.0
    h_g2 = t2 and _finite(sigma_capture) and float(sigma_capture) > 0.0
    h_g3 = t3 and _finite(kappa_compact) and float(kappa_compact) > 0.0
    h_g4 = t4 and _finite(rho_rigidity) and float(rho_rigidity) > 0.0
    h_g5 = t5 and _finite(alpha_alg) and float(alpha_alg) > 0.0
    h_g6 = t6 and _finite(a_floor) and float(a_floor) > 0.0
    h_gcoh_base = t7 and _finite(eps_coh) and float(eps_coh) >= 0.0
    h_gcoh = h_gcoh_base and ((abs(float(eps_coh)) < 1e-15) if strict_coh_zero else True)

    margin = None
    if all(
        _finite(x) for x in (kappa_hodge, sigma_capture, kappa_compact, rho_rigidity, alpha_alg, a_floor, eps_coh)
    ):
        margin = min(
            float(kappa_hodge),
            float(sigma_capture),
    ...
    h_gm = h_g1 and h_g2 and h_g3 and h_g4 and h_g5 and h_g6 and h_gcoh and _finite(margin) and float(margin) > 0.0

    gates = {
        "H_G1": "PASS" if h_g1 else "FAIL",
        "H_G2": "PASS" if h_g2 else "FAIL",
        "H_G3": "PASS" if h_g3 else "FAIL",
        "H_G4": "PASS" if h_g4 else "FAIL",
        "H_G5": "PASS" if h_g5 else "FAIL",
        "H_G6": "PASS" if h_g6 else "FAIL",
        "H_GCoh": "PASS" if h_gcoh else "FAIL",
        "H_GM": "PASS" if h_gm else "FAIL",
    }
    blockers = {
        "H_G1": [] if h_g1 else ["kappa_hodge missing/nonpositive or not theorem-level"],
        "H_G2": [] if h_g2 else ["sigma_capture missing/nonpositive or not theorem-level"],
        "H_G3": [] if h_g3 else ["kappa_compact missing/nonpositive or not theorem-level"],
        "H_G4": [] if h_g4 else ["rho_rigidity missing/nonpositive or not theorem-level"],
        "H_G5": [] if h_g5 else ["alpha_alg missing/nonpositive or not theorem-level"],
        "H_G6": [] if h_g6 else ["a_floor missing/nonpositive or not theorem-level"],
        "H_GCoh": [] if h_gcoh else ["eps_coh missing/invalid or strict-zero target failed"],
        "H_GM": [] if h_gm else ["strict margin <= 0 or upstream gates failed"],
    }
    normalized_gates = {
        "G1": gates["H_G1"],
        "G2": gates["H_G2"],
